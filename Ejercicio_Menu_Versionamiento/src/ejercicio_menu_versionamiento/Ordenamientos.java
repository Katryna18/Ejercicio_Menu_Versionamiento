
package ejercicio_menu_versionamiento;

import javax.swing.JOptionPane;

/**
 *
 * @author KatheH
 */
public class Ordenamientos {

    void opcCuatro() {
        
        String opc  ="";
        while(!opc.equalsIgnoreCase("C")){
         opc = JOptionPane.showInputDialog("***4.Ordenamientos***\n"+
                                           "A. Método Burbuja\n"+
                                           "B. Método Shell\n"+
                                           "C. Salir\n");
        
        switch(opc){
            case "A": //Método Burbuja
                burbuja();
                break;
                
            case "B": //Método Shell
                shell();
                break;
                
            case "C": //Salir
                JOptionPane.showMessageDialog(null, "Usted está regresando al menú principal");
                break;
                
            default:
                JOptionPane.showMessageDialog(null, "Por favor digita una opción valida");
        }
    }
}
    //MÉTODO BURBUJA
    public void burbuja(){
        int tamanio = Integer.parseInt(JOptionPane.showInputDialog("Por favor digita el tamaño del arreglo"));
        int arregloDatos [] = new int[tamanio];
        String opBurbuja = " ";
                while(!opBurbuja.equalsIgnoreCase("d")){
                    opBurbuja = JOptionPane.showInputDialog("A. Método Burbuja\n"+
                                                           "a. Cargar datos\n"+
                                                           "b. Mostrar datos\n"+
                                                           "c. Ordenar\n"+
                                                           "d. Salir");
                    
                    switch(opBurbuja){
                        case "a":
                            for(int f=0; f<arregloDatos.length;f++){
                                arregloDatos[f]=Integer.parseInt(JOptionPane.showInputDialog("Por favor ingresa un valor númerico en la posicion "+ f ));
                            }
                        break;
                        
                        case "b":
                            for(int j=0;j<arregloDatos.length;j++){
                                JOptionPane.showMessageDialog(null,"El valor en la posicion "+ j + " es : "+ arregloDatos[j] );
                            }
                            break;
                            
                        case "c":
                            int i, j, aux;
                            for (i = 0; i < arregloDatos.length - 1; i++) {
                                for (j = 0; j < arregloDatos.length - i - 1; j++) {
                                    if (arregloDatos[j + 1] < arregloDatos[j]) {
                                        aux = arregloDatos[j + 1];
                                        arregloDatos[j + 1] = arregloDatos[j];
                                        arregloDatos[j] = aux;
                                    }
                                }
                            }
                            JOptionPane.showMessageDialog(null, "Datos ordenados exitosamente!");
                            break;
                            
                        case "d":
                            JOptionPane.showMessageDialog(null, "Saliendo");
                           
                            break;
                            
                        default:
                            JOptionPane.showMessageDialog(null, "Digite una opcion valida");
                    }
                }
    }
    
    //MÉTODO SHELL
    public void shell(){ 
        int tamanio = Integer.parseInt(JOptionPane.showInputDialog("digite el tamaño del arreglo"));
        int arregloDatos [] = new int[tamanio];
        String opShell = "";
                while(!opShell.equalsIgnoreCase("d")){
                    opShell= JOptionPane.showInputDialog("B. Método Shell\n"+
                                                           "a. Cargar datos\n"+
                                                           "b. Mostrar datos\n"+
                                                           "c. Ordenar\n"+
                                                           "d. Salir");
                    
                    switch(opShell){
                        case "a":
                            for(int f=0; f<arregloDatos.length;f++){
                                arregloDatos[f]=Integer.parseInt(JOptionPane.showInputDialog("ingrese un valor númerico en la posicion "+ f ));
                            }
                        break;
                        
                        case "b":
                            for(int j=0;j<arregloDatos.length;j++){
                                JOptionPane.showMessageDialog(null,"el valor en la posicion "+ j + " es "+ arregloDatos[j] );
                            }
                            break;
                            
                        case "c":
                            int salto, aux, i;
                            boolean cambios;
                            for(salto=arregloDatos.length/2; salto!=0; salto/=2){
                                cambios=true;
                                while(cambios){ // Mientras se intercambie algún elemento
                                    cambios=false;
                                    for(i=salto; i< arregloDatos.length; i++) // se da una pasada
                                        if(arregloDatos[i-salto]>arregloDatos[i]){ // y si están desordenados
                                            aux=arregloDatos[i]; // se reordenan
                                            arregloDatos[i]=arregloDatos[i-salto];
                                            arregloDatos[i-salto]=aux;
                                            cambios=true; // y se marca como cambio.
                                            }
                                }
                            }
                            JOptionPane.showMessageDialog(null, "Datos ordenados Exitosamente!");
                            break;
                            
                        case "d":
                            JOptionPane.showMessageDialog(null, "Saliendo");
                            break;
                            
                        default:
                            JOptionPane.showMessageDialog(null, "Digite una opción valida");
                    }
                }
        
    }
    }
